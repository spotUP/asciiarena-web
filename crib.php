...
if (!is_logged_in()) {
	...
	if(isset($_POST['Save'])) {
		...
		if (isset($_POST['old_password']) && strlen($_POST['old_password']) > 0
			...
			if (md5($_POST['old_password']) !== fetchOne('select pwhash from users where id=:id', [ 'id' => $_user['id'] ])->pwhash) {
				$errors[] = 'old password is incorrect';
			}
			
			if (count($errors) == 0) {
				$ask="update users set pwhash=:pwhash where id=:id";
				$q = doQuery($ask,[ 'pwhash' => md5($_POST['new_password']), 'id' => $_user['id'] ]);
		...
		<div class="row amb-1">
			...
			<div class="col-3">
				<div class="form-check">
					<input class="form-check-input" type="checkbox" value="1" name="crt_effect" <?=($crt_effect === 'Y') ? 'checked' : '';?>>
				</div>
			</div>
		</div>
