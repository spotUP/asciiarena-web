						...
						$dirname = $dirname[ 0 ];
						{
							...
							foreach (fetchAll($ask, [":searchquery" => $searchquery]) as $row) {
								...
								<div class="row">
									...
									<div class="col-lg-4 green"><?=combinize($row->artists, $row->artist_ids, "/artists/", $row->artists)?></div>
									<div class="col-lg-4 yellow"><?=combinize($row->crews, $row->crew_ids, "/crews/", $row->crews)?></div>
								</div>
								<?php
							}
						}
							...
							<table class="table table-liquid mt-2">
								...
								<tbody>
									...
									foreach ($result as $row) {
										?>
										<tr>
											<td><a class="magenta" href="info_release.php?filename=<?=base64_encode($row->filename)?>"><?=myTruncate($row->name, 40, " ", "...")?></a>
											</td>
											<td class="green"><?=combinize($row->artists, $row->artist_ids, "/artists/", $row->artists)?></td>
											<td class="yellow"><?=combinize($row->crews, $row->crew_ids, "/crews/", $row->crews)?></td>
										</tr>
										<?php
									} ?>
								</tbody>
