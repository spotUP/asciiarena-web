...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const [rows, countRows] = await Promise.all([
      prisma.$queryRawUnsafe<AppRow[]>(
        `SELECT id, name, filename, author, filesize, year, month, day FROM apps ${whereSql} ORDER BY name ASC LIMIT ${perPage} OFFSET ${offset}`,
        ...params,
      ),
      prisma.$queryRawUnsafe<{ cnt: bigint | number }[]>(
        `SELECT COUNT(*) AS cnt FROM apps ${whereSql}`,
        ...params,
      ),
    ]);
    ...
    const data = rows.map((r) => ({
      ...
      html_url: r.filename ? `/application/${r.filename}` : null,
    }));
    return v1Ok(data, page, perPage, total, v1RateHeaders(rl.remaining));
  } catch {
    return v1Error("Failed to list apps", 500);
  }
}
