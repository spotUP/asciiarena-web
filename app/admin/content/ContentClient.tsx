...
function Section({ label, listApi, adminApi, detailPath }: { label: string; listApi: string; adminApi: string; detailPath: string }) {
  ...
  const search = async () => {
    ...
    const rows = await fetch(`${listApi}?filter=${encodeURIComponent(query)}&pagesize=30`)
      .then(r => r.json())
      .catch(() => []);
    setResults(rows);
    setEdits({});
        ...
        {results.length > 0 && (
          <>
            <div className="row lightgrey amb-1" style={{ borderBottom: "1px solid #444" }}>
              <div className="col-2">FILENAME</div>
            ...
            {results.map(item => {
              ...
              return (
                <div key={item.id} className="row amb-1 align-items-center">
                  ...
                  <div className="col-2">
                    <input type="text" className="form-control w-100" value={e.author ?? item.author ?? ""} onChange={ev => edit(item.id, "author", ev.target.value)} />
                  </div>
                  <div className="col-1">
                    <input type="number" className="form-control w-100" value={e.year ?? item.year ?? ""} onChange={ev => edit(item.id, "year", parseInt(ev.target.value) || null)} />
                  </div>
