...
// ANSI animation: split at terminal screen height (25 rows) so screen clears
// and row-count boundaries each start a new frame. The default splitRows is 27.
...
function ArchiveEntryRenderer({ filename, entry, ansiFont }: { filename: string; entry: string; ansiFont: string }) {
  ...
  // Initial load — splitRender at screen-height detects screen clears as frame boundaries
  useEffect(() => {
    ...
    if (isAnsi) {
      loadAnsiLove().then(api => {
        ...
        }, SPLIT_ROWS, { font: ansiFont, bits: "8", icecolors: 1, columns: 80, filetype: "ans" }, () => {
